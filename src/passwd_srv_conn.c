...
/**
 * Create socket to listen for client request
 *  API creates and bind socket and returns socket-descriptor created.
 *  If it failed to create and bind socket, return -1
 *
 *  @param sockaddr[OUT] sockaddr structure for UNIX domain socket
 *  @return socket       socket descriptor created
 */
int create_socket(struct sockaddr_un *sockaddr)
{
    int fdSocket = 0, size = 0, err = 0;
    ...
    size = sizeof(unix_sockaddr.sun_family) + strlen(unix_sockaddr.sun_path);
    if (0 != (err = unlink(unix_sockaddr.sun_path)))
    {
        /* TODO: logging for failure */
        return -1;
    }
    ...
    /* copy sockaddr structure before return */
    memcpy(sockaddr, &unix_sockaddr, sizeof(*sockaddr));
