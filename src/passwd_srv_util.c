 ...
 * @param useradd  add if true, delete otherwise
 */
static
struct spwd *create_user(const char *username, const char *groupname, int useradd)
{
    char useradd_comm[COMMAND_LENGTH];
    char user_ps_del[COMMAND_LENGTH];
    ...
    /* make sure that user has been created  or deleted*/
    if (NULL == (passwd_entry = find_password_info(username)))
    {
        memset(useradd_comm, 0, sizeof(useradd_comm));
        return NULL;
    }
