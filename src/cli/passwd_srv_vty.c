 ...
 */
static unsigned char *encrypt_msg_to_passwd_srv_d(passwd_srv_msg_t in,
                                                  int *msg_size)
{
    RSA *pubkey = NULL;
 ...
 */
static void
send_credential_to_passwd_server(const char* username, const char* groupname,
                                 const char* oldpass,
...
static int
is_native_user(const char *user)
{
    static char *native_user[MAX_TOTAL_USERS] = {ROOT_USER,
                                                 NETOP_USER,
                                                 ADMIN_USER};
    ...
    for (int itr = 0; itr < MAX_TOTAL_USERS; itr++)
    {
        if ((native_user[itr] != NULL) && !strcmp(native_user[itr], user))
        {
            return itr;
        }
    }
...
delete_user(const char *user)
{
    ...
    /* admin, netop and root are native users. dont delete those*/
    if (is_native_user(user) != -1)
    {
        vty_out(vty, "Permission denied. You can not delete default user.%s",
                VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
...
/* Function is not used right now but may be used in future
      ...
      USER_SHOW_STR)
{
    ...
    if (group_index == -1)
    {
        vty_out(vty,"Fetching user and group information failed.%s",
                VTY_NEWLINE);
        return CMD_ERR_NOTHING_TODO;
    }
    return CMD_SUCCESS;
