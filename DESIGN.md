...
Password server is a daemon that runs as root which serves as entry point to change
...
Password will be encrypted on the client-side using public key.  The password server
...
To secure password transfer from the client to password server, public/private
keys are used to send user information.

During password server initialization, public/private keys are generated using
openssl library.  The public key is stored in the file system which the client uses
