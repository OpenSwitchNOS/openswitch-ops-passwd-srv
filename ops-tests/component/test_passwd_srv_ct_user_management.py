...
def user_remove_default_users(ops1, step, user):
    step('### Test to verify default users cannot be removed ###')
    cmd = "user remove "+user
    out = ops1(cmd)
    assert 'Permission denied.' in out, "Test failed: default" \
        " user was deleted."
...
def test_vtysh_ct_user_management(topology, step):
    ...
    user_remove_default_users(ops1, step, "root")
    user_remove_default_users(ops1, step, "admin")
    user_remove_default_users(ops1, step, "netop")
